...
const emit = defineEmits(['update:visible', 'family-action'])

const drawerState = ref('no-group')
...
function updateState(family) {
  if (family) {
    drawerState.value = 'has-group'
  } else {
    drawerState.value = 'no-group'
  }
  newGroupName.value = ''
  joinCode.value = ''
}
