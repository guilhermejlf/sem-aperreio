...
const store = beneStore
let cycleTimer = null

const hasInsight = computed(() => !!store.currentInsight)
...
function showRandomInsight() {
  if (store.visible || !settingsStore.bene.showInsights) return
  const variants = ['neutral', 'info', 'success', 'warning']
  const weights = [0.35, 0.25, 0.25, 0.15]
  const variant = weightedPick(variants, weights)
  store.showInsight(variant)
}

function startCycle() {
  const cycle = FREQ_CYCLES[settingsStore.bene.frequency] || FREQ_CYCLES.normal
  cycleTimer = setInterval(() => {
    if (!store.visible && !store.currentInsight) {
      showRandomInsight()
    }
  }, cycle)
}
