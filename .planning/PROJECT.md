...
- Options API no Vue → **Migração em andamento 2026-05-29** — Fase 1+2+3+4+5: 23 componentes migrados (AppLoading, BeneAvatar, SkeletonGeneric, StatementGroup, SkeletonSettings, SkeletonProfile, SkeletonDashboard, StatementEmptyState, BeneInsight, BottomNav, OfflineFallback, ConfirmModal, StatementTimeline, StatementSkeleton, EmptyState, VerifyEmailView, InstallPrompt, ErrorBoundary, ModalBase, DashboardInsights, ForgotPasswordModal, LoginForm, StatementItem). usePagination composable criado. Restam ~26 componentes.
